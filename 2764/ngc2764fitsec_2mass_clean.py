...
def fit_ngc2764():
    """
    MGE fit to the 2MASS image for NGC2764.  
    """
    #### Set File:
    ...
    skylev = 423.6068115000   # Counts/pixel

    seesh  = 0.96  	      # Seeing shape parameter from the telescope.

    qBound = 0.42     	      # Lower q-bound

    MAGZP  = 20.1086          # This is the magnitude zero point listed in the fits header of the 
    Ak     = 0.012            # Galactic extinction parameter.  Find from NED

    D      = 39.6*10**6       # Distance to NGC 4476 in parsecs

    RCO    = 3800.00          # From the table in the overleaf document, this number needs to be in
                              # parsec.

    Mdyn   = 2.3*10**10       # From the table in the overleaf document
...
##### Find Galaxy:
    ...
    plt.pause(1)  # Allow plot to appear on the screen
    plt.savefig("n2764_pgp_2mass.png")
...
##### Do the actual MGE fit
    ...
    plt.pause(1)                      # Allow plot to appear on the screen
    plt.savefig("n2764_mfs_2mass.png") # Show contour plots of the results
    plt.clf()
    plt.subplot(121)
    ...
    mge_print_contours(img.clip(minlevel), f.theta, f.xpeak, f.ypeak, m.sol, scale=scale,
                       binning=4, sigmapsf=sigmapsf, magrange=5.0)
    ...
    plt.pause(1)  # Allow plot to appear on the screen
    plt.savefig("n2764_cp_2mass.png")
    plt.clf()
...
##### All printing to terminal:
    ...
    print('  Distance to NGC 2764 in parsecs: ', D)
    print('  parsec/pixel for NGC 2764: ', d)
